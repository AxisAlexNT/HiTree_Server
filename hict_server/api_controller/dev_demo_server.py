...
import threading
from typing import Any, NamedTuple, Optional, List, Dict, Tuple, Union
...
from flask import Blueprint, Flask, request, make_response, send_file, jsonify
...
class API_Server(FlaskView):
    api_server = Blueprint("API_Server", __name__, url_prefix="/v1")
    ...
    class API_Server_Options(NamedTuple):
        data_path: Path
    ...
    def __init__(
        ...
        self.chunked_file: Optional[ChunkedFile] = None
        self.colormap: Any = plt.get_cmap('Greens')
        self.transport_dtype: str = 'uint8'
        self.filename: Optional[str] = None
        self.fasta_filename: Optional[str] = None
        self.currentNormalizationSettings: NormalizationSettings = API_Server.DEFAULT_NORMALIZATION_SETTINGS
        self.currentContrastRange: ContrastRangeSettings = API_Server.DEFAULT_CONTRAST_RANGE
        self.currentMinSignalValue: Dict[int, Union[float, int]] = dict()
        self.currentMaxSignalValue: Dict[int, Union[float, int]] = dict()
        self.currentTileVersion: int = 0

    def update_options(self, options: API_Server_Options):
        self.update_data_path(options.data_path)
        self.update_mp_manager(options.mp_manager)
        
    def update_data_path(self, data_path: Path):
        self.data_path = data_path
        
    def update_mp_manager(self, mp_manager: multiprocessing.managers.SyncManager):
        self.mp_manager = mp_manager
        self.rlock: Union[threading.RLock, RLock] = self.mp_manager.RLock()
        def get_rlock():
            return self.rlock 
        self.chunked_file_lock: rwlock.RWLockWrite = rwlock.RWLockWrite(
            lock_factory=get_rlock
        )
        self.minMaxLock: rwlock.RWLockWrite = rwlock.RWLockWrite(
            lock_factory=get_rlock)
        self.versionLock: rwlock.RWLockWrite = rwlock.RWLockWrite(
            lock_factory=get_rlock)
    ...
    def generate_assembly_info(f: ChunkedFile) -> AssemblyInfo:
        return AssemblyInfo(
            API_Server.get_contig_descriptors(chunked_file),
            API_Server.get_scaffold_descriptors(chunked_file)
        )


    @api_server.post("/list_files")
    def list_files(self):
        files = list(
            sorted(map(lambda p: str(p.relative_to(self.data_path)), self.data_path.rglob("*.hdf5"))))
        response = flask.jsonify(files)
        response.headers['Access-Control-Allow-Origin'] = '*'
        response.status_code = 200
        return response

    @api_server.post("/open")
    def open_file(): #self):
        ...
        self.currentTileVersion = 0
        self.currentNormalizationSettings = API_Server.DEFAULT_NORMALIZATION_SETTINGS
        self.currentContrastRange = API_Server.DEFAULT_CONTRAST_RANGE
        self.chunked_file = ContactMatrixFacet.get_file_descriptor(
            str(self.data_path.joinpath(filename).resolve().absolute()),
        ...
        if fasta_filename is not None and fasta_filename != "":
            self.chunked_file.link_fasta(
                str(self.data_path.joinpath(self.fasta_filename).resolve().absolute()))
        with self.chunked_file_lock.gen_wlock() as cfl:
            ContactMatrixFacet.open_file(self.chunked_file)
    ...
    def generate_open_file_info(self) -> OpenFileResponseDTO:
        tile_size: int = int(max(self.chunked_file.dense_submatrix_size.values()))

        resolutions: List[int] = [int(r) for r in sorted(
            self.chunked_file.resolutions, reverse=True)]
        with self.chunked_file_lock.gen_wlock() as cfl:
            response: OpenFileResponse = OpenFileResponse(
                ...
                tile_size,
                self.generate_assembly_info(chunked_file),
                [self.chunked_file.contig_tree.root.subtree_length_bins[r] for r in
                    resolutions] if self.chunked_file.contig_tree.root is not None else []
            )
    ...
    @api_server.post("/close")
    def close_file(): #self):
        if self.chunked_file is not None:
            with self.chunked_file_lock.gen_wlock() as cfl:
                ContactMatrixFacet.close_file(self.chunked_file)
    ...
    @api_server.get("/get_assembly_info")
    def get_assembly_info(): #self):
        if self.chunked_file is None or self.chunked_file.state != ChunkedFile.FileState.OPENED:
            raise Exception("File is not opened?")
        with self.chunked_file_lock.gen_wlock() as cfl:
            assemblyInfo: AssemblyInfo = API_Server.generate_assembly_info(self.chunked_file)
